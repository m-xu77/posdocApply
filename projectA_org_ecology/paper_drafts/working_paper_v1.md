...
Four robustness checks discipline the headline findings.

First, **page-count normalization**. A concern is that the post-2018 volume increase reflects thicker yearbooks rather than more substantive activity. Normalizing event counts by yearbook page count for the central yearbook series gives a P1→P3 per-page ratio of 1.3×, against a raw per-year ratio of 4.9×. The thicker-yearbook account explains some but not most of the increase; the substantive intensification signal survives.

Second, **confidence stratification**. Re-running the actor-share calculation on the subset of events coded with `confidence = high` (n = 9,217) preserves the local-government collapse (24.1% in P1 → 6.8% in P3) and the research-institute rise (0.3% → 19.4%). The directional findings are not artifacts of low-confidence extractions.

Third, **pub_year-vs-data_year sensitivity**. A subset of events carries a non-trivial gap between publication year and data year, which can be created by retrospective compilation. Excluding events with `|pub_year - data_year| > 2` (n = 1,134 excluded) leaves the headline pattern unchanged. The compilation-lag account does not explain the actor-share shifts.

Fourth, **alternative diversity indices**. Computing Simpson's (1 − D) and Blau's heterogeneity index in parallel with Shannon entropy gives a qualitatively identical trajectory (peak at 2017–2018; plateau through 2022; 2023 compression). Inverse-Simpson values rise from 5.0 in 2013 to 7.4 in 2018, then drop to 3.6 in 2023; the same story in different units. The diversity narrative does not depend on the choice of index.

The full robustness tables are in the supplementary appendix. None of the checks reverse the headline patterns; one (page-count normalization) reduces the magnitude of the volume-intensification signal but leaves its sign intact.
...
The third is that the *governance-frame dominance* simply reflects the coding scheme's bias toward governance categories. This too has substance: coordination/supervision is one of eleven categories, and it absorbs everything that does not fit one of the more specific bundles. Re-running the analysis with the coordination/supervision category excluded gives a governance-frame share of 39.6% against the infrastructure-frame share of 25.0% — a smaller gap but still a clear governance lean. The qualitative point — *the Chinese model coordinates and trains as much as it builds* — survives the alternative coding.
